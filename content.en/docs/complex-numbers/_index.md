...
*Fig.2.1 Graph of  f(x) = x^2 - 1*

![](https://placehold.co/600x400)
*Fig.2.2 Graph of  f(x) = x^2 + 1*
...
Two complex numbers  z_{1} = x_{1} + iy_{1} and  z_{2} = x_{2} + iy_{2} are said to be equal if and only if  \mathrm{Re}(z_{1}) = \mathrm{Re}(z_{2}) and  \mathrm{Im}(z_{1}) = \mathrm{Im}(z_{2}). That is  x_{1} = x_{2} and  y_{1} = y_{2}.
...
1. If  z_{1} = 1 - 3i,  z_{2} = -4i, and  z_{3} = 5, show that
 ...
 (z_{1} + z_{2}) + z_{3} = z_{1} + (z_{2} + z_{3}) (ii)  (z_{1}z_{2})z_{3} = z_{1}(z_{2}z_{3}).

2. If  z_{1} = 3,  z_{2} = -7i, and  z_{3} = 5 + 4i, show that

 (z_{1}z_{2} + z_{3}) = z_{1}z_{2} + z_{1}z_{3} (ii)  (z_{1} + z_{2})z_{3} = z_{1}z_{3} + z_{2}z_{3}.
...
3. If  z_{1} = 2 - i and  z_{2} = -4 + 3i, find the inverse of  z_{1}z_{2} and  \frac{z_{1}}{z_{2}}.
...
Now consider the triangle shown in figure with vertices  O, z_{1} or  z_{2}, and  z_{1} + z_{2}. We know from geometry that the length of the side of the triangle corresponding to the vector  z_{1} + z_{2} cannot be greater than the sum of the lengths of the remaining two sides. This is the reason for calling the property as "Triangle Inequality".
...
**Property** The distance between the two points  z_{1} and  z_{2} in complex plane is  |z_{1} - z_{2}|

If  z_{1} = x_{1} + i y_{1} and  z_{2} = x_{2} + i y_{2}, then
...
The distance between the two points  z_{1} and  z_{2} in complex plane is  |z_{1} - z_{2}|

If we consider origin,  z_{1} and  z_{2} as vertices of a triangle, by the similar argument we have
...
For any two complex numbers  z_{1} and  z_{2}, we have  |z_{1}z_{2}| = |z_{1}||z_{2}|
...
If  z_{1}, z_{2}, and  z_{3} are complex numbers such that  \left|z_{1}\right| = \left|z_{2}\right| = \left|z_{3}\right| = \left|z_{1} + z_{2} + z_{3}\right| = 1 find the value of  \left|\frac{1}{z_{1}} + \frac{1}{z_{2}} + \frac{1}{z_{3}}\right|.
...
Therefore,  \overline{z_{1}} = \frac{1}{z_{1}}, \overline{z_{2}} = \frac{1}{z_{2}}, and  \overline{z_{3}} = \frac{1}{z_{3}} and hence
...
To find the lower bound and upper bound use  \left| |z_{1}| - |z_{2}| \right| \leq |z_{1} + z_{2}| \leq |z_{1}| + |z_{2}|.
...
Let  z_{1}, z_{2}, and  z_{3} be complex numbers such that  \left|z_{1}\right| = \left|z_{2}\right| = \left|z_{3}\right| = r > 0 and  z_{1} + z_{2} + z_{3} \neq 0
...
Therefore  |z - z_{0}| = r is the complex form of the equation of a circle.
...
Suppose  z_{1}, z_{2}, and  z_{3} are the vertices of an equilateral triangle inscribed in the circle  |z| = 2. If  z_{1} = 1 + i\sqrt{3}, then find  z_{2} and  z_{3}.

**Solution**

 |z| = 2 represents the circle with centre  (0, 0) and radius 2.

Let  A, B, and  C be the vertices of the given triangle. Since the vertices  z_{1}, z_{2}, and  z_{3} form an equilateral triangle inscribed in the circle  |z| = 2, the sides of this triangle  AB, BC, and  CA subtend  \frac{2\pi}{3} radians (120 degrees) at the origin (circumcenter of the triangle).

(The complex number  z e^{i\theta} is a rotation of  z by  \theta radians in the counter clockwise direction about the origin.)

Therefore, we can obtain  z_{2} and  z_{3} by the rotation of  z_{1} by  \frac{2\pi}{3} and  \frac{4\pi}{3} respectively.

Given that

 \overline{OA} = z_{1} = 1 + i\sqrt{3};
 \overline{OB} = z_{1} e^{i \frac{2\pi}{3}} = (1 + i\sqrt{3}) e^{i \frac{2\pi}{3}}
...
- Properties of complex conjugates
   ...
   \overline{z_{1} - z_{2}} = \overline{z_{1}} - \overline{z_{2}}
   \overline{z_{1} z_{2}} = \overline{z_{1}} \overline{z_{2}}
   \overline{\left(\frac{z_{1}}{z_{2}}\right)} = \frac{\overline{z_{1}}}{\overline{z_{2}}}, z_{2} \neq 0
...
- Properties of Modulus of a complex number
   ...
   \left|\frac{z_{1}}{z_{2}}\right| = \frac{|z_{1}|}{|z_{2}|}, z_{2} \neq 0
